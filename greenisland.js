...
window.addEventListener("scroll", () => {
  ...
  if (scrollPosition > 8500 && stage === 0) {
  ...
  // 第二階段：第二張圖和第二段文字
  else if (scrollPosition > 8900 && stage === 1) {
  ...
  // 第三階段：第三張圖和第三段文字
  else if (scrollPosition > 9300 && stage === 2) {
  ...
  // 第四階段：第四張圖和第四段文字
  else if (scrollPosition > 9700 && stage === 3) {
    ...
    stage = 4;
  } else if (scrollPosition > 10800 && stage === 4) {
    flyingTexts[0].style.opacity = "0";
...
document.addEventListener("scroll", function () {
  ...
  const scrollWrapper = document.getElementById("sec5-scroll-wrapper");
  ...
  // 確保捲動範圍在 [0, 1] 之間
  const scrollProgress = Math.min(1, Math.max(0, (scrollTop - containerTop) / scrollRange));
  const maxScrollLeft = scrollWrapper.scrollWidth - window.innerWidth;

  // 避免超出範圍
  if (maxScrollLeft > 0) {
    scrollWrapper.style.transform = `translateX(-${scrollProgress * maxScrollLeft}px)`;
  }
});
